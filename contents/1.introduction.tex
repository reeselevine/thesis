...
One of the fundamental aspects of any programming framework is how it defines operations on memory. In programming frameworks where multiple operations runs in parallel, e.g., on GPUs, these definitions must also specify the semantics when multiple processing elements (commonly called \textit{threads}) concurrently access the same underlying memory. This set of definitions and rules is called a \textit{memory consistency specification} (MCS). Complicating matters further, modern processors exploit reordering, buffering, and deep cache hierarchies, where memory operations do not necessarily become visible to other threads in intuitive orders~\cite{nagarajan2020primer,gharachorloo1991performance,sarkar2011understanding,sarkar2009semantics}. To deal with this complexity, CPU architectures like x86~\cite{owens2009x86} and ARM~\cite{alglave2009semantics} have formalized their MCSs, with languages like C++~\cite{batty2011mathematizing} and Java~\cite{manson2005java} also formalizing MCSs that abstract over the underlying hardware.

In comparison, cross-platform GPU frameworks have been developed more recently and target an even more diverse set of underlying architectures from many vendors with behaviors that are not well understood. While previous work formalized the MCS of virtual ISAs from specific GPU vendors like NVIDIA~\cite{lustig2019ptx} and developed a formal MCS for cross-platform frameworks like OpenCL~\cite{batty2016overhauling}, the empirically observable behaviors of GPUs from different vendors have been shown to vary substantially and numerous bugs in GPU software and hardware stacks have been found throughout the years~\cite{alglave2015gpu,alglave2010fences,kirkham2020foundations}. These results highlight a critical gap between cross-platform MCSs and the behaviors of the underlying GPUs.
...
\subfloat[Message Passing]{\resizebox{.7\columnwidth}{!}{\mprlxintrolitmustest} \label{fig:intro-mprlx}} \\
\subfloat[Message Passing with Fences]{\resizebox{.7\columnwidth}{!}{\mprelacqintrolitmustest} \label{fig:intro-mprelacq}}
