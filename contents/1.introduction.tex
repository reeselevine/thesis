...
In comparison, cross-platform GPU frameworks have been developed more recently and target an even more diverse set of underlying architectures from many vendors with behaviors that are not well understood. While previous work formalized the MCS of virtual ISAs from specific GPU vendors like NVIDIA~\cite{lustig2019ptx} and developed a formal MCS for cross-platform frameworks like OpenCL~\cite{formal-opencl}, the empirically observable behaviors of GPUs from different vendors have been shown to vary substantially and numerous bugs in GPU software and hardware stacks have been found throughout the years~\cite{gpu-concurrency, fences, gpu-foundations}. These results highlight a critical gap between cross-platform MCSs and the behaviors of the underlying GPUs.
...
The guarantees provided by MCSs are directly intertwined with a programming model's overall guarantees around memory safety. Data races can lead to unexpected and unintuitive behaviors, including memory safety violations, as shown in \mychap\ref{chap:saferace}. While previous work on CPU-based languages has sought to constrain the effects of data races~\cite{bounding-races} and develop safe languages that run in the browser~\cite{sharedarraybuffer,weak-wasm}, no research has focused on the complexity of maintaining GPU memory safety in the presence of data races.
...
\begin{itemize}
  ...
  \item \textbf{Reese Levine}, Mingun Cho, Devon McKee, Andi Quinn, and Tyler Sorensen. 2023. \textit{GPUHarbor: Testing GPU Memory Consistency at Large (Experience Paper)}. In Proceedings of the 32nd ACM SIGSOFT International Symposium on Software Testing and Analysis (ISSTA 2023)~\cite{gpuharbor}.
  \begin{itemize}
...
\paragraph{Co-Author}
\begin{itemize}
  \item Sean Siddens, Sanya Srivastava, \textbf{Reese Levine}, Josiah Dykstra, and Tyler Sorensen. 2026. \textit{Memory DisOrder: Memory Re-orderings as a Timerless Side-channel}. arXiv preprint arXiv:2601.08770~\cite{siddens2026disorder}.
  \begin{itemize}
